...
class RequirementWorkflow:
    ...
    def _validation_node(self, state: WorkflowState) -> Dict[str, Any]:
        ...
        try:
            validation_results = self.validation_agent.validate(state["requirement_data"])
            
            return {
                "validation_results": validation_results,
                "messages": state["messages"] + [{"role": "system", "content": f"Validation complete: {validation_results['is_valid']}"}],
            ...
            return {
                ...
                "messages": state["messages"] + [{"role": "system", "content": f"Error during validation: {str(e)}"}]
            }
    ...
    def _formatting_node(self, state: WorkflowState) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "messages": state["messages"] + [{"role": "system", "content": "Formatting complete"}],
            ...
            return {
                ...
                "current_node": ERROR_NODE,
                "messages": state["messages"] + [{"role": "system", "content": f"Error during formatting: {str(e)}"}]
            }
    ...
    def _document_creation_node(self, state: WorkflowState) -> Dict[str, Any]:
        ...
        try:
            document_results = self.document_agent.create_documents(state["formatted_prd"])
            
            # Extract document links
            doc_links = {}
            if "documents_created" in document_results:
                if "google_doc" in document_results["documents_created"]:
                    doc_links["google_doc_url"] = document_results["documents_created"]["google_doc"].get("doc_url", "")
                if "jira_ticket" in document_results["documents_created"]:
                    doc_links["jira_ticket_url"] = document_results["documents_created"]["jira_ticket"].get("ticket_url", "")
            
            return {
                "document_results": document_results,
                "doc_links": doc_links,
                "messages": state["messages"] + [{"role": "system", "content": "Document creation complete"}],
            ...
            return {
                ...
                "current_node": ERROR_NODE,
                "messages": state["messages"] + [{"role": "system", "content": f"Error during document creation: {str(e)}"}]
            }
    ...
    def _design_document_node(self, state: WorkflowState) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "wiki_url": wiki_url,
                "messages": state["messages"] + [{"role": "system", "content": "Design document creation complete"}],
            ...
            return {
                ...
                "current_node": ERROR_NODE,
                "messages": state["messages"] + [{"role": "system", "content": f"Error during design document creation: {str(e)}"}]
            }
    ...
    def _complete_node(self, state: WorkflowState) -> Dict[str, Any]:
        ...
        return {
            "messages": state["messages"] + [{"role": "system", "content": "Workflow complete"}],
    ...
    def _error_node(self, state: WorkflowState) -> Dict[str, Any]:
        ...
        return {
            "messages": state["messages"] + [{"role": "system", "content": f"Workflow error: {error_message}"}],
            "current_node": ERROR_NODE
        }
    ...
    def execute(self, requirement_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        result = {
            ...
            "links": {
                "google_doc": final_state.get("doc_links", {}).get("google_doc_url", ""),
                "jira_ticket": final_state.get("doc_links", {}).get("jira_ticket_url", ""),
